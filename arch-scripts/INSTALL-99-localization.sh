...
Use=true
VariantList=,"
ls "${targetFile}" 2>&1 1>/dev/null && { echo "${targetFile} is installed... Applying localization."; sudo -u $ARCH_USERNAME kwriteconfig5 --group Formats --key LANG --file ${plasmaLocale} ${ARCH_LANG}; } || echo "||"
ls "${targetFile}" 2>&1 1>/dev/null && { sudo -u $ARCH_USERNAME echo "$kxkbrc" >> $kxkbrcFile }
