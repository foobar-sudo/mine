...
patchez=("lucjan" "linux-tkgs" "cacule")
function lucjan() {
gitdir="."
...
unused=("*sep" "*-dev*" "*-trunk*" "*ll*" "*futex2-stable*" "*futex2*" "*futex-trunk*" "*futex-patches*")
#unused=()
needed=("android" "cpu" "btrfs" "zstd-patches" "zstd-dev-patches" "xanmod-patches" "arch-patches" "pf-patches" "block-patches" "bfq-patches" "lqx-patches" "v4l2loopback-patches" "ntfs3")
...
options=('!strip')
_srcname=linux
source=("\${_srcname}::git+https://git.kernel.org/pub/scm/linux/kernel/git/stable/\${_srcname}.git#branch=linux-\${_major}.y"
config::https://git.archlinux.org/svntogit/packages.git/plain/trunk/config?h=packages/linux
$(for patch in "${patchez[@]}"; do eval "$patch"; done)
)
sha256sums=(\$(for src in "\${source[@]}"; do printf "%s\n" "\${src}"; done))
EOF
...
prepare() {
  ...
  for src in "${source[@]}"; do
    ...
    #patch -Np1 < "../$src"
    #OUT="$(patch -Np1 --forward < "../$src")" || echo "Applying patch ${OUT}..." | grep "Skipping patch" -q || (echo "$OUT" && false)
    OUT="$(patch -Np1 --forward < "../$src")" || echo "Applying patch ${OUT}..." | grep "Skipping patch" -q || (echo "$OUT" && false)
  done
